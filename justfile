# Development and release tasks. `just dev` for everything, `just wheel` for the
# release artifact, which must be built in the manylinux container: a library
# compiled against a host glibc is not portable.
#
# Recipes avoid shell chaining (`&&`) so they work under both sh and PowerShell;
# use the working-directory attribute instead.
