...
def test_lazy_lib_ignores_attribute_probes() -> None:
    ...
    pytest, inspect, copy and pickle all probe dunders; those probes must not
    dlopen ~70 MB (or raise NativeLibraryNotFound where it was never built).
    """
    fresh = _lib._LazyLib()
...
def test_loads_with_closed_standard_handles(tmp_path: Path, native_available: bool) -> None:
    ...
    A daemonised proxy has no usable standard handles, and the panic a dependency
    then raises inside wazero aborts the *host* process.
    """
