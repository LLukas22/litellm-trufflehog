...
def blocking_error(exc: BaseException) -> dict:
    ...
    HTTPException when fastapi is available, SecretDetected otherwise.
    """
...
def test_block_message_never_contains_the_secret(guard: TrufflehogGuardrail) -> None:
    """An error body echoing the secret would re-leak it, often into client logs."""
    with pytest.raises(BLOCKED) as exc_info:
...
def test_redact_falls_back_to_block_when_unredactable(scanner: Scanner, monkeypatch) -> None:
    """Fail closed: a secret that cannot be located cannot be masked."""
...
def test_stream_holdback_is_advertised(guard: TrufflehogGuardrail) -> None:
    """LiteLLM withholds this tail so a secret cannot be split across chunks."""
    result = apply(guard, [CLEAN_TEXT])
...
#
# A skipped detector yields no findings; treating that as "clean" would let a
# secret through precisely when scanning is broken.
...
def test_truncation_reason_is_distinct(native_available: bool) -> None:
    """Truncation and detector failure must be distinguishable by the caller."""
    small = Scanner(profile="minimal", max_bytes=32)
...
def test_invalid_on_detection_rejected(scanner: Scanner) -> None:
    """Config comes from YAML, so invalid values must be rejected at runtime."""
...
def test_integrates_with_real_litellm_base_class(scanner: Scanner) -> None:
    """Pin the litellm integration: a rename or moved base class must fail loudly
    rather than silently fall back to the stub and never be invoked."""
...
def test_streaming_hook_blocks_on_split_secret(guard: TrufflehogGuardrail) -> None:
    """The secret is split across deltas, so only the overlapping window finds it."""
    chunks = [
