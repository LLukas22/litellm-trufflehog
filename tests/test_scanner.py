...
def test_multipart_credential_reports_every_part(scanner: Scanner) -> None:
    """AWS reports Raw=key-id and RawV2="key-id:secret"; both must be located."""
...
def test_verification_is_off_by_default(scanner: Scanner) -> None:
    """Verification requires network calls, which must never happen implicitly."""
...
def test_warmup_reports_no_errors(scanner: Scanner) -> None:
    """Warm-up initialises the RE2/wazero runtime at construction.

    Regression: it used to happen inside the first scan and could panic there,
    silently dropping a detector.
    """
...
def test_concurrent_scans_are_safe(scanner: Scanner) -> None:
    """ctypes drops the GIL during the native call, so these genuinely overlap."""
