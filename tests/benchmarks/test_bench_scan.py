...
Where `just bench-go` measures the Go scanner alone, these add what a caller pays
on top: the ctypes call, the UTF-8 encode, JSON decoding and dataclass building.
"""
...
def test_scan_decoy(benchmark, bench_scanner: Scanner, decoy_payloads, size: str) -> None:
    ...
    Each keyword defeats the prefilter and forces the regexes to run: the
    realistic worst case for a proxy in front of a coding assistant.
    """
...
# ---------------------------------------------------------------------------
#
# Three layers over the same input, grouped so pytest-benchmark prints the ratios
# and the deltas isolate the wrapper's cost from the scan's:
#   encode_only  -> the str -> bytes conversion alone
...
# The same split on a payload with findings, where the JSON report is far larger.
...
def test_scan_thread_scaling(
    ...
    ctypes releases the GIL, so MB/s should rise with thread count; flat means
    scans are serialising and a thread pool buys nothing.
    """
    text = clean_payloads["16kb"]
...
def test_scanner_construction(benchmark, native_available: bool, profile: str) -> None:
    """Paid once per worker at startup. Excluded from the throughput table: it is
    not a per-byte cost."""
