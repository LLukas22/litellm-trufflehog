...
Where `test_bench_scan.py` measures a single `scan()`, this measures the whole
per-request path: `apply_guardrail` (one `asyncio.to_thread` per text) and the
streaming hook, whose cost depends on chunk count rather than response size.
"""
...
#: Characters per streamed delta. A model emits about one token per chunk, which
#: is what makes chunk counts large.
DELTA_CHARS = 20
...
def test_guardrail_many_texts(
    ...
    One `asyncio.to_thread` per text, so this prices dispatch overhead - which is
    per-text, not per-byte - against the scan itself.
    """
...
# ---------------------------------------------------------------------------
#
# StreamScanner rescans an overlap tail per chunk, so N deltas cost roughly
# N x (overlap + delta) bytes instead of one pass. The baseline below is the same
# bytes in a single scan, so the ratio in this group is the amplification factor.
...
def test_stream_delta_size(benchmark, bench_scanner: Scanner, clean_payloads, delta: int) -> None:
    """Cost against delta size at the default overlap: smaller deltas mean more
    chunks, and the per-chunk rescan makes the total superlinear in chunk count."""
    text = clean_payloads["2kb"]
...
def test_first_scan_after_construction(benchmark, native_available: bool) -> None:
    ...
    `get_scanner` caches, so this is paid once; later requests pay only the scan.
    """
