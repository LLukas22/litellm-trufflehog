...
A model streams a response in small deltas, so a secret can straddle a chunk
boundary: ``"ghp_abc"`` in one chunk and ``"def..."`` in the next. Scanning each
delta in isolation would miss it.

Prefer LiteLLM's own mechanism where you can: returning ``stream_holdback_chars``
from ``apply_guardrail`` makes the proxy withhold trailing characters so a match
is never split, which is both simpler and exact. This module is for the
``async_post_call_streaming_iterator_hook`` path, where you own the chunk loop
and need to decide per chunk whether to keep streaming.

The approach mirrors trufflehog's own chunker, which overlaps successive chunks
by ``DefaultPeekSize`` (3 KiB) for exactly this reason.
"""
...
#: split across a boundary without being seen whole in some window.
...
class StreamScanner:
    ...
    def feed(self, chunk: str) -> ScanReport:
        ...
        The returned report's spans are offsets into the scan *window* (overlap
        tail + chunk), not into the overall stream, so they are suitable for
        masking the window but not for indexing the full response.
        """
        if not chunk:
            return ScanReport()
    ...
    def flush(self) -> ScanReport:
        """Finish the stream. The overlap tail has already been scanned, so this
        only exists to release state and report nothing new."""
        self._tail = ""
        return ScanReport()
    ...
    def _only_new(self, report: ScanReport) -> ScanReport:
        """Filter out secrets already reported for this stream.

        Because windows overlap, the same secret is seen in consecutive scans;
        deduplicating on the content hash keeps the caller's view clean.
        """
        fresh: list[Finding] = []
